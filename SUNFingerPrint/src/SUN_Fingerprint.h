#ifndef SUN_FINGERPRINT_H
#define SUN_FINGERPRINT_H
...
#define FINGERPRINT_HISPEEDSEARCH                                              \
  0x04 //!< Asks the sensor to search for a matching fingerprint template to the
...
struct SUN_Fingerprint_Packet {
  ...
  */
  ...
  SUN_Fingerprint_Packet(uint8_t type, uint16_t length, uint8_t *data) {
...
class SUN_Fingerprint {
public:
#if defined(__AVR__) || defined(ESP8266) || defined(FREEDOM_E300_HIFIVE1)
  SUN_Fingerprint(SoftwareSerial *ss, uint32_t password = 0x0);
#endif
  SUN_Fingerprint(HardwareSerial *hs, uint32_t password = 0x0);
  SUN_Fingerprint(Stream *serial, uint32_t password = 0x0);
  ...
  void writeStructuredPacket(const SUN_Fingerprint_Packet &p);
  uint8_t getStructuredPacket(SUN_Fingerprint_Packet *p,
